...
namespace BAMC
{
    static class BAMCByteDecoder
    {
        ...
        public static int ReadVarInt(this byte[] source, int StartPosition, out int value)
        {
            value = 0;
            var count = 0;
            while (count < 5)
        ...
        public static int ReadString(this byte[] source, int StartPosition, out string value)
        {
            int index = source.ReadVarInt(StartPosition, out int length);
            value = Encoding.UTF8.GetString(source, index, length);
            return index + length;
        }
        public static int ReadUShort(this byte[] source, int StartPosition, out ushort value)
        {
            value = 0;
